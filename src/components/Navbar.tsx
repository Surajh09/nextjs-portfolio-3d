...
export default function Navbar(): JSX.Element {
    ...
    return (
        <>
                    ...
                    <div className='flex-row items-center justify-between hidden text-lg font-bold md:flex md:gap-6 lg:gap-8'>
                        <ul className='flex flex-row md:gap-6 lg:gap-8 justify-evenly'>
                            {navlinks.map((link, index) => (
                                <li key={index}>
                                    <Link
                                        className={`${pathname === link.href ? 'text-white' : 'text-gray-400'
                                            } p-4`}
                                        href={link.href}
                                    >
                                        {link.name}
                        ...
                        <a className='px-6 py-2 text-white gradient-btn rounded-xl' href="/" download="Suraj Hemnani Resume">Resume</a>
                    </div>
            ...
            {/* mobile menu */}
            <div
                className={`${isMenuOpen ? "top-0" : "-translate-y-full"} fixed top-0 w-screen h-screen transition-all duration-500 ease-in-out z-40 bg-gray-800`}
